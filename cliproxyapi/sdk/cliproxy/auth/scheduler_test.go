...
func TestManager_PickNextMixed_CodexXHighRequiresProPlan(t *testing.T) {
	t.Parallel()

	model := "scheduler-codex-xhigh-requires-pro"
	registerSchedulerModels(t, "codex", model, "codex-xhigh-plus", "codex-xhigh-pro")

	manager := NewManager(nil, &RoundRobinSelector{}, nil)
	manager.executors["codex"] = schedulerTestExecutor{}
	if _, errRegister := manager.Register(context.Background(), &Auth{ID: "codex-xhigh-plus", Provider: "codex", Attributes: map[string]string{"plan_type": "plus"}}); errRegister != nil {
		t.Fatalf("Register(codex-xhigh-plus) error = %v", errRegister)
	}
	if _, errRegister := manager.Register(context.Background(), &Auth{ID: "codex-xhigh-pro", Provider: "codex", Attributes: map[string]string{"plan_type": "pro"}}); errRegister != nil {
		t.Fatalf("Register(codex-xhigh-pro) error = %v", errRegister)
	}

	opts := cliproxyexecutor.Options{
		Metadata: map[string]any{cliproxyexecutor.ReasoningEffortMetadataKey: "xhigh"},
	}
	got, _, provider, errPick := manager.pickNextMixed(context.Background(), []string{"codex"}, model, opts, nil)
	if errPick != nil {
		t.Fatalf("pickNextMixed() error = %v", errPick)
	}
	if got == nil {
	...
	if got.ID != "codex-xhigh-pro" {
		t.Fatalf("pickNextMixed() auth.ID = %q, want codex-xhigh-pro", got.ID)
	}
}

func TestManager_PickNextMixed_CodexHighPrefersPlusAndFallsBackToPro(t *testing.T) {
	t.Parallel()

	model := "scheduler-codex-high-prefers-plus"
	registerSchedulerModels(t, "codex", model, "codex-high-plus", "codex-high-pro", "codex-high-cooldown-plus", "codex-high-fallback-pro")

	opts := cliproxyexecutor.Options{
		Metadata: map[string]any{cliproxyexecutor.ReasoningEffortMetadataKey: "high"},
	}

	manager := NewManager(nil, &RoundRobinSelector{}, nil)
	manager.executors["codex"] = schedulerTestExecutor{}
	if _, errRegister := manager.Register(context.Background(), &Auth{ID: "codex-high-plus", Provider: "codex", Attributes: map[string]string{"plan_type": "plus"}}); errRegister != nil {
		t.Fatalf("Register(codex-high-plus) error = %v", errRegister)
	}
	if _, errRegister := manager.Register(context.Background(), &Auth{ID: "codex-high-pro", Provider: "codex", Attributes: map[string]string{"plan_type": "pro"}}); errRegister != nil {
		t.Fatalf("Register(codex-high-pro) error = %v", errRegister)
	}
	got, _, _, errPick := manager.pickNextMixed(context.Background(), []string{"codex"}, model, opts, nil)
	if errPick != nil {
		t.Fatalf("pickNextMixed() high error = %v", errPick)
	}
	if got == nil || got.ID != "codex-high-plus" {
		t.Fatalf("pickNextMixed() high auth = %#v, want codex-high-plus", got)
	}

	fallback := NewManager(nil, &RoundRobinSelector{}, nil)
	fallback.executors["codex"] = schedulerTestExecutor{}
	if _, errRegister := fallback.Register(context.Background(), &Auth{
		ID:             "codex-high-cooldown-plus",
		Provider:       "codex",
		Attributes:     map[string]string{"plan_type": "plus"},
		Unavailable:    true,
		NextRetryAfter: time.Now().Add(time.Hour),
	}); errRegister != nil {
		t.Fatalf("Register(codex-high-cooldown-plus) error = %v", errRegister)
	}
	if _, errRegister := fallback.Register(context.Background(), &Auth{ID: "codex-high-fallback-pro", Provider: "codex", Attributes: map[string]string{"plan_type": "pro"}}); errRegister != nil {
		t.Fatalf("Register(codex-high-fallback-pro) error = %v", errRegister)
	}
	got, _, _, errPick = fallback.pickNextMixed(context.Background(), []string{"codex"}, model, opts, nil)
	if errPick != nil {
		t.Fatalf("pickNextMixed() high fallback error = %v", errPick)
	}
	if got == nil || got.ID != "codex-high-fallback-pro" {
		t.Fatalf("pickNextMixed() high fallback auth = %#v, want codex-high-fallback-pro", got)
	}
}

func TestManager_PickNextMixed_CodexMaxWithoutProErrors(t *testing.T) {
	t.Parallel()

	model := "scheduler-codex-max-without-pro"
	registerSchedulerModels(t, "codex", model, "codex-max-plus")

	manager := NewManager(nil, &RoundRobinSelector{}, nil)
	manager.executors["codex"] = schedulerTestExecutor{}
	if _, errRegister := manager.Register(context.Background(), &Auth{ID: "codex-max-plus", Provider: "codex", Attributes: map[string]string{"plan_type": "plus"}}); errRegister != nil {
		t.Fatalf("Register(codex-max-plus) error = %v", errRegister)
	}

	opts := cliproxyexecutor.Options{
		Metadata: map[string]any{cliproxyexecutor.ReasoningEffortMetadataKey: "max"},
	}
	got, _, provider, errPick := manager.pickNextMixed(context.Background(), []string{"codex"}, model, opts, nil)
	if errPick == nil {
		t.Fatalf("pickNextMixed() error = nil, auth = %#v provider = %q; want pro-capable auth error", got, provider)
	}
	if got != nil || provider != "" {
		t.Fatalf("pickNextMixed() auth/provider = %#v/%q, want nil/empty", got, provider)
	}
}
