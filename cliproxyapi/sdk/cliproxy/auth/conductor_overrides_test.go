...
func TestManager_MarkResult_TransientErrorCooldownDefault(t *testing.T) {
	...
	if state.NextRetryAfter.IsZero() {
		t.Fatal("expected transient error cooldown to keep the default")
	}
...
func TestManager_MarkResult_InvalidRequestDoesNotCooldownAuth(t *testing.T) {
	m := NewManager(nil, nil, nil)

	auth := &Auth{
		ID:       "auth-invalid-request",
		Provider: "codex",
	}
	if _, errRegister := m.Register(context.Background(), auth); errRegister != nil {
		t.Fatalf("register auth: %v", errRegister)
	...
	m.MarkResult(context.Background(), Result{
		...
		Error: &Error{
			HTTPStatus: http.StatusBadRequest,
			Message:    `{"error":{"message":"Unknown parameter: input[20].namespace","code":"unknown_parameter"}}`,
		},
	})
	...
	if updated.Status == StatusError {
		t.Fatalf("expected request-scoped 400 not to set auth status error")
	}
	if !updated.NextRetryAfter.IsZero() {
