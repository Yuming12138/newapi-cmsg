...
func TestFileTokenStore_List_CodexPlanTypeAttribute(t *testing.T) {
	ctx := context.Background()
	baseDir := t.TempDir()
	path := filepath.Join(baseDir, "codex.json")

	if err := os.WriteFile(path, []byte(`{"type":"codex","plan_type":"team"}`), 0o600); err != nil {
		t.Fatalf("seed auth file: %v", err)
	}
	...
	if got := auths[0].Attributes["plan_type"]; got != "team" {
		t.Fatalf("plan_type attribute = %q, want team", got)
	}
}
