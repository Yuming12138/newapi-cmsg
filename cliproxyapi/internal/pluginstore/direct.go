...
func (c Client) DownloadArtifact(ctx context.Context, artifact Artifact) ([]byte, error) {
	plan := NormalizeInstallPlan(InstallPlan{Type: InstallTypeDirect, Artifacts: []Artifact{artifact}})
	...
	data, errDownload := c.get(ctx, artifact.URL, "application/octet-stream", RequestKindArtifact)
	if errDownload != nil {
