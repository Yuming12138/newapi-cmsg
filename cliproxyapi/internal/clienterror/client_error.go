...
func IsRequestFault(status int, err error) bool {
	...
	// pairs them with a generic invalid_request_error body.
...
// IsItemNotPersisted matches the upstream 404 raised when a request references
// an item that was never persisted because store=false.
