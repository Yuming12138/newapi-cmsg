...
func TestIsRequestFault(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			err:    errors.New("Item with id 'rs_test' not found. Items are not persisted when `store` is set to false. Try again with `store` set to true, or remove this item from your input."),
			want:   true,
		},
		{
		...
		{
			name:   "authentication failure remains credential failure",
			status: http.StatusUnauthorized,
			err:    errors.New(`{"error":{"code":"invalid_request_error","message":"Authentication failed","type":"authentication_error"}}`),
		},
		{
			name:   "payment status overrides generic request error code",
			status: http.StatusPaymentRequired,
			err:    errors.New(`{"error":{"message":"Insufficient Balance","type":"unknown_error","code":"invalid_request_error"}}`),
		},
		{
			name:   "rate limit status overrides generic request error code",
			status: http.StatusTooManyRequests,
			err:    errors.New(`{"error":{"message":"Rate Limit Reached","type":"unknown_error","code":"invalid_request_error"}}`),
		},
