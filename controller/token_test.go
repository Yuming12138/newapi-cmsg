...
func TestUpdateTokenCanClearGroupAndDisableCrossGroupRetry(t *testing.T) {
	db := setupTokenControllerTestDB(t)
	token := seedToken(t, db, 1, "clearable-token", "clear1234group5678")
	token.Group = "auto"
	...
	if updated.Group != "" {
		t.Fatalf("expected empty group after update, got %q", updated.Group)
	}
	if updated.CrossGroupRetry {
		t.Fatalf("expected cross_group_retry to be false after update")
	}
	if strings.Contains(recorder.Body.String(), token.Key) {
		t.Fatalf("update response leaked raw token key: %s", recorder.Body.String())
	}
