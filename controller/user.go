...
func GetSelf(c *gin.Context) {
	...
	quotaRequest, _ := service.GetSelfUserQuotaRequestStatus(user.Id, user.Quota)
...
func RequestUserQuota(c *gin.Context) {
	id := c.GetInt("id")
	user, err := model.GetUserById(id, false)
	if err != nil {
		common.ApiError(c, err)
	...
	status, err := service.CreateSelfUserQuotaRequest(user)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func ListUserQuotaRequests(c *gin.Context) {
	requests, err := service.ListPendingUserQuotaRequests()
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func ApproveUserQuotaRequest(c *gin.Context) {
	review := service.UserQuotaRequestReview{
		RequestID:     c.Param("request_id"),
		Approve:       true,
		AdminID:       c.GetInt("id"),
		AdminUsername: c.GetString("username"),
	}
	request, err := service.ReviewUserQuotaRequest(review)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func RejectUserQuotaRequest(c *gin.Context) {
	review := service.UserQuotaRequestReview{
		RequestID:     c.Param("request_id"),
		Approve:       false,
		AdminID:       c.GetInt("id"),
