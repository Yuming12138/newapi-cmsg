...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		model.BeginChannelAttempt(channel.Id)
		switch relayFormat {
		case types.RelayFormatOpenAIRealtime:
		...
		if newAPIError == nil {
			model.FinishChannelAttempt(channel.Id, true, time.Since(attemptStart), 0)
			relayInfo.LastError = nil
			return
		}

		model.FinishChannelAttempt(channel.Id, false, time.Since(attemptStart), newAPIError.StatusCode)
		newAPIError = service.NormalizeViolationFeeError(newAPIError)
...
// upstream channel attempt. A later selector failure only means that retry has
// no remaining channel; replacing the upstream error with it hides actionable
// causes such as auth_unavailable and changes a 502 into a misleading 500.
func preferLastRelayError(channelSelectionError *types.NewAPIError, lastRelayError *types.NewAPIError) *types.NewAPIError {
	if lastRelayError != nil {
		return lastRelayError
	}
	return channelSelectionError
}
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError) {
	...
	service.HandleChannelFailure(c, channelError, err)
	...
	if constant.ErrorLogEnabled && types.IsRecordErrorLog(err) {
		...
		service.AppendChannelSchedulerAdminInfo(c, adminInfo)
...
func RelayTask(c *gin.Context) {
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		model.FinishChannelAttempt(channel.Id, false, time.Since(attemptStart), taskErr.StatusCode)
