...
func ClearChannelAffinityForRequest(c *gin.Context, reason string) bool {
	if c == nil {
		return false
	}
	...
	// Once the affinity target is known bad, allow the current request to fall
	// back to normal routing and prevent future requests with the same key from
	// sticking to the disabled channel.
	c.Set(ginKeyChannelAffinitySkipRetry, false)

	cache := getChannelAffinityCache()
	deletedMap, err := cache.DeleteMany([]string{cacheKey})
	if err != nil {
		common.SysError(fmt.Sprintf("channel affinity cache delete failed: key=%s, reason=%s, err=%v", cacheKey, reason, err))
		return false
	}
...
func markChannelAffinityRecordSkipped(c *gin.Context, reason string, channelID int, channelPriority int64, maxPriority int64) {
	if c == nil {
		return
	}
	anyInfo, ok := c.Get(ginKeyChannelAffinityLogInfo)
	...
	info["record_skipped"] = true
	info["record_skip_reason"] = reason
	info["record_channel_id"] = channelID
	info["record_channel_priority"] = channelPriority
	info["max_schedulable_priority"] = maxPriority
	c.Set(ginKeyChannelAffinityLogInfo, info)
}

func channelAffinityPriorityModels(modelName string) []string {
	modelName = strings.TrimSpace(modelName)
	if modelName == "" {
		return nil
	}
	models := []string{modelName}
	normalized := strings.TrimSpace(ratio_setting.FormatMatchingModelName(modelName))
...
func maxSchedulableChannelAffinityPriority(group string, modelName string) (int64, bool) {
	group = strings.TrimSpace(group)
	if group == "" {
		return 0, false
	}
	for _, candidateModel := range channelAffinityPriorityModels(modelName) {
		var abilities []model.Ability
		err := model.DB.Where(&model.Ability{
			Group:   group,
			Model:   candidateModel,
			Enabled: true,
		}).Find(&abilities).Error
		if err != nil {
			common.SysError(fmt.Sprintf("channel affinity priority query failed: group=%s model=%s err=%v", group, candidateModel, err))
			return 0, false
		}
		found := false
...
func shouldSkipChannelAffinityRecordForFallback(c *gin.Context, channelID int) bool {
	if c == nil || channelID <= 0 {
		return false
	}
	meta, ok := getChannelAffinityMeta(c)
	...
	maxPriority, ok := maxSchedulableChannelAffinityPriority(meta.UsingGroup, meta.ModelName)
	if !ok {
		return false
	}
	if channelPriority >= maxPriority {
		return false
	}
	markChannelAffinityRecordSkipped(c, "fallback_priority", channelID, channelPriority, maxPriority)
	return true
}
...
func RecordChannelAffinity(c *gin.Context, channelID int) {
	...
	if shouldSkipChannelAffinityRecordForFallback(c, channelID) {
		return
	}
