...
func DispatchPlatformUpdate(platform constant.TaskPlatform, taskChannelM map[int][]string, taskM map[string]*model.Task) {
	...
	case constant.TaskPlatformImage:
		// Playground image tasks are completed by the local background relay.
...
func taskNeedsUpdate(oldTask *model.Task, newTask dto.SunoDataResponse) bool {
	...
	if (oldTask.Status == model.TaskStatusFailure || oldTask.Status == model.TaskStatusSuccess || oldTask.Status == model.TaskStatusCancelled) && oldTask.Progress != "100%" {
		return true
	}
