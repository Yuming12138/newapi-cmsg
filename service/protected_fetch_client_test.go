...
func TestInitHttpClientKeepsGeneralProviderClientUnprotected(t *testing.T) {
	originalHTTPClient := httpClient
	originalProtectedClient := ssrfProtectedHTTPClient
	t.Cleanup(func() {
		httpClient = originalHTTPClient
	...
	generalTransport, ok := GetHttpClient().Transport.(*http.Transport)
	require.True(t, ok)
	require.Nil(t, generalTransport.DialContext)

	_, ok = ssrfProtectedHTTPClient.Transport.(*ssrfProtectedRoundTripper)
	require.True(t, ok)
}

func TestProtectedFetchRedirectRejectsPrivateDestination(t *testing.T) {
	configureSSRFTestFetchSetting(t)
