...
export async function getQuotaRequests(): Promise<
  ApiResponse<QuotaRequestRecord[]>
> {
  const res = await api.get('/api/user/quota_requests')
  return res.data
}
...
export async function reviewQuotaRequest(
  payload: QuotaRequestReviewRequest & { request_id: string }
): Promise<ApiResponse> {
  const res = await api.post(
    `/api/user/quota_requests/${payload.request_id}/${payload.approve ? 'approve' : 'reject'}`
  )
  return res.data
}
