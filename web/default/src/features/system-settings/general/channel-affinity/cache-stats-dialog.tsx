...
export function CacheStatsDialog(props: Props) {
  ...
  const rows = useMemo(() => {
    ...
    const keyValue = String(props.target?.key_value || '').trim()
        ...
        {loading ? (
          ...
          <div className='space-y-2'>
            {rows.map((row) => (
              <div
                key={row.key}
                className='flex items-start justify-between gap-4 border-b pb-1 text-sm'
              >
                <span className='text-muted-foreground shrink-0'>{row.key}</span>
                <span className='flex min-w-0 items-start gap-2 text-right'>
                  <span
                    className={`min-w-0 break-all font-medium ${row.mono ? 'font-mono text-xs' : ''}`}
                  >
                    {row.value}
                  </span>
                  {row.copyable ? (
                    <button
                      ...
                      onClick={() => copyValue(String(row.value))}
                    >
                      {t('Copy')}
                    </button>
                  ) : null}
