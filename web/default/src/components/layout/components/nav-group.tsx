...
function SidebarMenuLink({ item, href }: { item: NavLink; href: string }) {
  ...
  const isExternal = item.external || isExternalUrl(url)
      ...
      <CollapsibleContent className='CollapsibleContent'>
        <SidebarMenuSub>
          {item.items.map((subItem) => {
            ...
            const isExternal = subItem.external || isExternalUrl(url)

            return (
              <SidebarMenuSubItem key={subItem.title}>
                <SidebarMenuSubButton
                  isActive={isExternal ? false : checkIsActive(href, subItem)}
                  render={
                    isExternal ? (
                      <a
                        href={url}
        ...
        <DropdownMenuContent side='right' align='start' sideOffset={4}>
          <DropdownMenuGroup>
            ...
            {item.items.map((sub) => {
              ...
              const isExternal = sub.external || isExternalUrl(url)
