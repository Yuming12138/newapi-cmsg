...
import TokenConfigModal from './modals/TokenConfigModal';
...
function TokensPage() {
  ...
  const openTokenConfigModalRef = useRef(null);
  const tokensData = useTokensData(
    (key) => openFluentNotificationRef.current?.(key),
    (key) => openCCSwitchModalRef.current?.(key),
    (record) => openTokenConfigModalRef.current?.(record),
  );
  ...
  const [tokenConfigVisible, setTokenConfigVisible] = useState(false);
  const [tokenConfigRecord, setTokenConfigRecord] = useState(null);
  ...
  function openTokenConfigModal(record) {
    setTokenConfigRecord(record || null);
    setTokenConfigVisible(true);
  }
  openTokenConfigModalRef.current = openTokenConfigModal;
  ...
  return (
    <>
      ...
      <TokenConfigModal
        visible={tokenConfigVisible}
        onClose={() => setTokenConfigVisible(false)}
        token={tokenConfigRecord}
        fetchTokenKey={tokensData.fetchTokenKey}
