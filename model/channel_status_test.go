...
func TestChannelInfoValueUsesPortableJSONText(t *testing.T) {
	info := ChannelInfo{
		IsMultiKey:   true,
		MultiKeySize: 2,
	...
	require.True(t, ok, "JSON values must use text instead of PostgreSQL bytea")
	assert.Contains(t, text, `"is_multi_key":true`)
	assert.NotContains(t, text, `\\x`)

	for _, stored := range []driver.Value{text, []byte(text)} {
