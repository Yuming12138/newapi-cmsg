...
func logQuotaDataCache(params QuotaDataLogParams) {
	key := fmt.Sprintf("%d-%s-%s-%d-%s", params.UserID, params.Username, params.ModelName, params.CreatedAt, params.NodeName)
...
func SaveQuotaDataCache() {
	...
	for _, quotaData := range CacheQuotaData {
		...
		DB.Table("quota_data").Where("user_id = ? and username = ? and model_name = ? and created_at = ? and node_name = ?",
			quotaData.UserID, quotaData.Username, quotaData.ModelName, quotaData.CreatedAt, quotaData.NodeName).First(quotaDataDB)
		if quotaDataDB.Id > 0 {
			//quotaDataDB.Count += quotaData.Count
			//quotaDataDB.Quota += quotaData.Quota
			//DB.Table("quota_data").Save(quotaDataDB)
			increaseQuotaData(quotaData.UserID, quotaData.Username, quotaData.ModelName, quotaData.Count, quotaData.Quota, quotaData.CreatedAt, quotaData.TokenUsed, quotaData.NodeName)
		} else {
...
func increaseQuotaData(userId int, username string, modelName string, count int, quota int, createdAt int64, tokenUsed int, nodeName string) {
	err := DB.Table("quota_data").Where("user_id = ? and username = ? and model_name = ? and created_at = ? and node_name = ?",
		userId, username, modelName, createdAt, nodeName).Updates(map[string]interface{}{
