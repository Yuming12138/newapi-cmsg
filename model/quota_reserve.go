...
if unlimited == 'true' or unlimited == '1' then
  redis.call('HSET', KEYS[1], 'RemainQuota', 0)
else
  redis.call('HINCRBY', KEYS[1], 'RemainQuota', tonumber(ARGV[1]))
end
redis.call('HINCRBY', KEYS[1], 'UsedQuota', -tonumber(ARGV[1]))
...
func cacheTryReserveTokenQuota(id int, key string, amount int64) (cacheQuotaResult, error) {
	result, err := common.RDB.Eval(context.Background(), tokenQuotaReserveScript,
		[]string{getTokenCacheKey(key)}, amount, id, common.GetTimestamp(), tokenCacheSchemaVersion).Int()
	return quotaResultFromLua(result, err)
}
...
func persistTokenQuotaDelta(id int, delta int) error {
	...
	result := DB.Model(&Token{}).Where("id = ?", id).Updates(
		map[string]interface{}{
			"remain_quota":  gorm.Expr("CASE WHEN unlimited_quota THEN 0 ELSE remain_quota + ? END", delta),
...
func recordUnlimitedTokenQuota(id int, quota int) error {
	if common.BatchUpdateEnabled {
	...
	if unlimited {
		if common.RedisEnabled {
			if _, cacheErr := cacheApplyTokenQuotaDelta(id, key, int64(-quota)); cacheErr != nil {
				common.SysLog("failed to update unlimited token quota cache: " + cacheErr.Error())
			}
		}
		return true, recordUnlimitedTokenQuota(id, quota)
	}
	if !common.RedisEnabled {
