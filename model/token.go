...
// Update persists all editable token fields, including zero values such as
// an empty group or disabled cross-group retry.
func (token *Token) Update() (err error) {
	// Raise the fence before writing so an in-flight DB reader cannot publish
	// a pre-mutation snapshot after the cache entry has been invalidated.
	...
	return DB.Model(token).Updates(updates).Error
}
