...
func GetChannel(group string, model string, retry int, excludedIDs ...map[int]struct{}) (*Channel, error) {
	...
	for priorityIdx := retry; priorityIdx < len(priorities); priorityIdx++ {
		var abilities []Ability
		query := DB.Where(commonGroupCol+" = ? and model = ? and enabled = ? and priority = ?", group, model, true, priorities[priorityIdx])
		...
		for _, ability := range abilities {
			if _, blocked := excluded[ability.ChannelId]; blocked {
				continue
			}
			if blocked, _ := IsChannelTemporarilyUnschedulable(ability.ChannelId); blocked {
				continue
			}
		...
		selected := selectChannelByRuntimeScore(candidates)
		if selected != nil {
			return selected, nil
		}
	}
