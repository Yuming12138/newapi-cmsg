...
const responsesTurnStateKey = "mimo_responses_turn_state"
...
func handleResponsesChatResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*dto.Usage, *types.NewAPIError) {
	...
	if !ok {
		return nil, types.NewOpenAIError(errors.New("missing mimo responses turn state"), types.ErrorCodeBadResponse, http.StatusInternalServerError)
	}
