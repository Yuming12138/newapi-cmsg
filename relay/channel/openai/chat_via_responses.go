...
func OaiResponsesToChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	var chatCompatResp dto.OpenAITextResponse
	if err := common.Unmarshal(body, &chatCompatResp); err == nil && isChatCompletionCompatResponse(&chatCompatResp) {
		usage := &chatCompatResp.Usage
		if usage == nil || usage.TotalTokens == 0 {
			text := extractChatCompletionText(&chatCompatResp)
			usage = service.ResponseText2Usage(c, text, info.UpstreamModelName, info.GetEstimatePromptTokens())
			chatCompatResp.Usage = *usage
		}
		if chatCompatResp.Id == "" {
			chatCompatResp.Id = chatId
		}
		if chatCompatResp.Model == "" {
			chatCompatResp.Model = info.UpstreamModelName
		}
		if err := writeChatCompletionCompatResponse(c, info, resp, &chatCompatResp); err != nil {
			return nil, err
		}
		return usage, nil
	}
	...
	if err := writeChatCompletionCompatResponse(c, info, resp, chatResp); err != nil {
		return nil, err
	}
...
func isChatCompletionCompatResponse(resp *dto.OpenAITextResponse) bool {
	if resp == nil {
		return false
	}
	if strings.HasPrefix(resp.Object, "chat.completion") {
		return true
	}
	return len(resp.Choices) > 0
}

func extractChatCompletionText(resp *dto.OpenAITextResponse) string {
	if resp == nil {
		return ""
	}
...
func writeChatCompletionCompatResponse(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response, chatResp *dto.OpenAITextResponse) *types.NewAPIError {
	var (
		responseBody []byte
		err          error
	)
...
func OaiResponsesToChatStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	sendToolCallDelta := func(callID string, name string, argsDelta string) bool {
		...
		if outputText.Len() > 0 && info.RelayFormat != types.RelayFormatClaude {
			// Keep legacy chat-compatible behavior for non-Claude clients. Claude streams
			// can contain assistant text followed by tool_use blocks in the same message.
			return true
		}
		if !sendStartIfNeeded() {
			return false
	...
	helper.StreamScannerHandler(c, resp, info, func(data string, sr *helper.StreamResult) {
		...
		case "response.completed":
			...
			if !sentStop {
				...
				finishReason := "stop"
				if sawToolCall && (outputText.Len() == 0 || info.RelayFormat == types.RelayFormatClaude) {
