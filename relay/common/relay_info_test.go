...
func TestRelayInfoMeteredUsageConcurrentApplyIsIdempotent(t *testing.T) {
	info := &RelayInfo{}
	var recorded int64
	...
	for i := 0; i < 32; i++ {
		...
		go func() {
			defer wg.Done()
			err := info.ApplyMeteredUsage(1000, func(delta int) error {
				atomic.AddInt64(&recorded, int64(delta))
				return nil
			})
			if err != nil {
				errs <- err
	...
	require.Equal(t, int64(1000), recorded)
}
